...
import { problemLabels, type IssueDraft } from "../types/issue";
      ...
      <div className="bg-surface rounded-xl border border-border p-5 shadow-sm">
        <div className="flex items-start justify-between mb-4 pb-4 border-b border-border gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-xs bg-warning/20 text-warning-foreground px-3 py-1 rounded-full font-medium">
              Awaiting expert
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            {draft.location && (
              <div className="text-xs text-muted-foreground mt-2">
                Pinned in {draft.location.room} · south wall near hallway
                opening · compared against 2023 capture.
              </div>
            )}
          </div>
          <div className="space-y-3 min-w-0">
            ...
            {draft.measurement && (
              <div>
                <div className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
                  <Ruler className="size-3" aria-hidden /> MEASUREMENT
                </div>
            ...
            {draft.tags.length > 0 && (
              <div>
                <div className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
                  <Tag className="size-3" aria-hidden /> FLAGS
                </div>
            ...
            {draft.photoAttached && (
              <div>
                ...
                <div className="h-16 rounded-md bg-gradient-to-br from-stone-300 to-stone-500 flex items-center justify-center text-xs text-white/90">
                  IMG_2043.jpg
                </div>
              </div>
            )}
